...
import { suspiciousApiPatterns, recommendations, analystNotes } from '../../shared/mock/insights';
  ...
  return (
    <Box>
      ...
      <SimpleGrid columns={{ base: 1, xl: 2 }} spacing="16px">
        ...
        <GlassCard p="20px"><SectionTitle>ЗАМЕТКИ АНАЛИТИКОВ</SectionTitle><VStack align="stretch">{analystNotes.map((note) => <Box key={note.id}><Text fontWeight="500">{note.author} · <Text as="span" fontFamily="mono" color={COLORS.textMuted} fontSize="11px">{formatDate(note.timestamp)}</Text></Text><Text color={COLORS.textSecondary} fontSize="13px">{note.body}</Text></Box>)}</VStack></GlassCard>
      </SimpleGrid>
    </Box>
  );
}
